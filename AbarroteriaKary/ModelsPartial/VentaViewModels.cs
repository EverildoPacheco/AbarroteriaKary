...
// ModelsPartial/VentasViewModels.cs
using System;
using System.Collections.Generic;
using System.ComponentModel.DataAnnotations;
using Microsoft.AspNetCore.Mvc.ModelBinding.Validation;
...
namespace AbarroteriaKary.ModelsPartial
{
    ...
    /// <summary>
    /// VM principal para crear una venta desde la pantalla “Nueva Venta”.
    /// </summary>
    public class VentaViewModel : IValidatableObject
    {
        // ---------- Identificadores y contexto ----------
        ...
        [ValidateNever]
        public string UsuarioNombre { get; set; } = string.Empty; // solo UI

        // ---------- Encabezado (tabla VENTA) ----------
        [Display(Name = "Fecha")]
        public DateTime FechaVenta { get; set; } = DateTime.Now;

        // Importante: NO Required aquí. El controlador normaliza vacío/“CF” → cliente CF real.
        [Display(Name = "Cliente")]
        public string ClienteId
        {
            get => _clienteId;
        ...
        // Para listados (si ya lo leyó de BD)
        [ValidateNever]
        public decimal? TotalDb { get; set; }

        // Total visible
        [Display(Name = "Total"), DataType(DataType.Currency)]
        public decimal Total => TotalDb.HasValue ? Math.Round(TotalDb.Value, 2) : CalcularTotal();
        ...
        [Display(Name = "Efectivo recibido"), DataType(DataType.Currency)]
        [Range(0, 999999999999.99)]
        public decimal? EfectivoRecibido { get; set; }

        [Display(Name = "Cambio"), DataType(DataType.Currency)]
        public decimal Cambio => Math.Max(0, Math.Round((EfectivoRecibido ?? 0) - Total, 2));
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            if (string.IsNullOrWhiteSpace(SesionId))
                yield return new ValidationResult(
                    "No hay caja aperturada. No se puede registrar la venta.",
                    new[] { nameof(SesionId) });
            ...
            if (Lineas != null)
            {
                for (int i = 0; i < Lineas.Count; i++)
                {
                    ...
                    // Precio debe ser > 0 (exigencia del negocio)
                    if (ln.PrecioUnitario <= 0)
                        yield return new ValidationResult($"El precio unitario de la línea {i + 1} debe ser mayor a Q0.00.", new[] { nameof(Lineas) });

                    // Si vino StockDisponible desde UI, validar contra él
                    if (ln.StockDisponible > 0 && ln.Cantidad > ln.StockDisponible)
                        yield return new ValidationResult(
                            $"La cantidad en la línea {i + 1} supera el stock disponible ({ln.StockDisponible:0.##}).",
                            new[] { nameof(Lineas) });
                }
            }

            // Cobro básico en efectivo (si lo usa)
            if (EfectivoRecibido.HasValue && EfectivoRecibido.Value < Total)
                yield return new ValidationResult(
                    "El efectivo recibido no cubre el total de la venta.",
                    new[] { nameof(EfectivoRecibido) });

            // Si usa múltiples recibos (otros métodos), la suma debe igualar al total
            if (Recibos != null && Recibos.Count > 0)
            {
                decimal suma = 0m;
                foreach (var r in Recibos) suma += Math.Max(0, r.Monto);
                if (Math.Round(suma, 2) != Math.Round(Total, 2))
                    yield return new ValidationResult(
                        "La suma de los recibos debe ser igual al total de la venta.",
                        new[] { nameof(Recibos) });
            }
        }
    }

    /// <summary>Línea de venta (detalle).</summary>
    public class VentaDetalleItemVM
    {
        [ValidateNever] public string? DetalleVentaId { get; set; } // correlativo DETV…
        [ValidateNever]
        ...
        [Display(Name = "Código")] public string? CodigoProducto { get; set; }
        [Display(Name = "Nombre")] public string? NombreProducto { get; set; }
        [Display(Name = "Imagen")] public string? ImagenUrl { get; set; }

        [Required, Display(Name = "Cantidad")]
        [Range(0.01, 9999999, ErrorMessage = "La cantidad debe ser mayor a 0.")]
        public decimal Cantidad { get; set; } = 1;

        /// <summary>PVP vigente (PRECIO_HISTORICO). Visible al cliente.</summary>
        [Required, Display(Name = "Precio unitario (PVP)"), DataType(DataType.Currency)]
        [Range(0.01, 9999999, ErrorMessage = "El precio unitario debe ser mayor a Q0.00.")]
        public decimal PrecioUnitario { get; set; } = 0;
        ...
        // -------- Costeo/Consumo (interno, para KARDEX) --------
        [ValidateNever] public List<LoteConsumoVM> LotesConsumidos { get; set; } = new();

        [DataType(DataType.Currency)]
        [Display(Name = "Costo de la línea")]
        public decimal? CostoTotal { get; set; }

        // Info auxiliar que se usa para validar en servidor
        [Display(Name = "Stock disponible")] public decimal StockDisponible { get; set; } = 0m;
        [Display(Name = "Próximo a vencer")] public DateTime? ProximoVencimiento { get; set; }
    }

    /// <summary>Un recibo (pago) aplicado a la venta (tabla RECIBO).</summary>
    public class ReciboItemVM
    {
        [ValidateNever] public string? ReciboId { get; set; } // correlativo REC…
        [ValidateNever] public string? VentaId { get; set; }
        ...
        [Required, Display(Name = "Monto"), DataType(DataType.Currency)]
    ...
    // DTOs auxiliares para búsquedas y UI
    public class ProductoVentaBusquedaItemVM
    {
        public string ProductoId { get; set; } = string.Empty;
        public string CodigoProducto { get; set; } = string.Empty;
        ...
        public string? DescripcionProducto { get; set; }
        public string? ImagenUrl { get; set; }
        public string? InfoRapida { get; set; }
    }
    ...
    public class ProductoVentaSeleccionadoVM
    {
        ...
        [Display(Name = "Precio unitario (PVP)"), DataType(DataType.Currency)]
        [Range(0.01, 9999999, ErrorMessage = "El precio unitario debe ser mayor a Q0.00.")]
        public decimal PrecioUnitario { get; set; } = 0;
    }
    ...
    public class LoteConsumoVM
    {
        [Display(Name = "Inventario")] public string InventarioId { get; set; } = string.Empty;
        [Display(Name = "Producto")] public string ProductoId { get; set; } = string.Empty;
        [Display(Name = "Lote")] public string? LoteCodigo { get; set; }
        [Display(Name = "Vence")] public DateTime? FechaVencimiento { get; set; }

        [Display(Name = "Cantidad (sale)")]
        [Range(0.01, 9999999, ErrorMessage = "La cantidad debe ser mayor a 0.")]
        public decimal Cantidad { get; set; }

        [Display(Name = "Costo unitario (lote)"), DataType(DataType.Currency)]
        public decimal CostoUnitario { get; set; }
    }
