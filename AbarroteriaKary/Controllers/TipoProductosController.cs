...
namespace AbarroteriaKary.Controllers
{
    public class TipoProductosController : Controller
    {
        ...
        public TipoProductosController(KaryDbContext context, ICorrelativoService correlativos, IAuditoriaService auditoria)
        {
            _context = context;
        ...
        public async Task<IActionResult> Index(string? estado, string? q = null, string? fDesde = null, string? fHasta = null, int page = 1, int pageSize = 10)
        {
            ...
            var qry = _context.TIPO_PRODUCTO
                .AsNoTracking()
                .Where(c => !c.ELIMINADO);
            ...
            if (!string.IsNullOrWhiteSpace(q))
            {
                var term = $"%{q.Trim()}%";
                qry = qry.Where(c =>
                    EF.Functions.Like(c.TIPO_PRODUCTO_ID, term) ||
                    EF.Functions.Like(c.TIPO_PRODUCTO_NOMBRE, term) ||
                    (c.TIPO_PRODUCTO_DESCRIPCION != null && EF.Functions.Like(c.TIPO_PRODUCTO_DESCRIPCION, term))
                );
            }
            ...
            var proyectado = qry
                .OrderBy(c => c.TIPO_PRODUCTO_NOMBRE)
                .Select(c => new TipoProductoViewModel
                {
                    TipoProductoId = c.TIPO_PRODUCTO_ID,
                    TipoProductoNomre = c.TIPO_PRODUCTO_NOMBRE,
                    TipoProductoDescripcion = c.TIPO_PRODUCTO_DESCRIPCION,
                    ESTADO = c.ESTADO,
        ...
        public async Task<IActionResult> Details(string id)
        {
            ...
            var entidad = await _context.TIPO_PRODUCTO
                .AsNoTracking()
                //.Include(p => p.AREA)
                .FirstOrDefaultAsync(r => r.TIPO_PRODUCTO_ID == id);

            if (entidad is null) return NotFound();

            // 3) Proyección a ViewModel (lo que usaremos en la vista)
            var vm = new TipoProductoViewModel
            {
                TipoProductoId = entidad.TIPO_PRODUCTO_ID,
                TipoProductoNomre = entidad.TIPO_PRODUCTO_NOMBRE,
                TipoProductoDescripcion = entidad.TIPO_PRODUCTO_DESCRIPCION,
        ...
        public async Task<IActionResult> Create(CancellationToken ct)
        {
            var vm = new TipoProductoViewModel
            {
                ...
                TipoProductoId = await _correlativos.PeekNextTipoProductoIdAsync(ct),
                ESTADO = "ACTIVO",
                EstadoActivo = true,
        ...
        public async Task<IActionResult> Create(TipoProductoViewModel vm, CancellationToken ct)
        {
            // 1) Sincroniza checkbox -> cadena ESTADO
            vm.RefrescarEstadoDesdeBool();

            // 2) Normalización básica (evita espacios/valores nulos)
            vm.TipoProductoNomre = (vm.TipoProductoNomre ?? string.Empty).Trim();
            vm.TipoProductoDescripcion = vm.TipoProductoDescripcion?.Trim();

            // 3) Validación servidor
            if (!ModelState.IsValid)
            {
                // Reponer "preview" si se perdió
                if (string.IsNullOrWhiteSpace(vm.TipoProductoId))
                    vm.TipoProductoId = await _correlativos.PeekNextTipoProductoIdAsync(ct);
            ...
            var duplicado = await _context.CATEGORIA.AnyAsync(c =>
                !c.ELIMINADO && c.CATEGORIA_NOMBRE == vm.TipoProductoNomre, ct);

            if (duplicado)
            {
                ModelState.AddModelError(nameof(vm.TipoProductoNomre), "Ya existe una Tipo Producto con ese nombre.");
                if (string.IsNullOrWhiteSpace(vm.TipoProductoId))
                    vm.TipoProductoId = await _correlativos.PeekNextTipoProductoIdAsync(ct);
                return View(vm);
            }
            ...
            try
            {
                ...
                var nuevoId = await _correlativos.NextSubTipoProductoIdAsync(ct);

                var entidad = new TIPO_PRODUCTO
                {
                    TIPO_PRODUCTO_ID = nuevoId,
                    TIPO_PRODUCTO_NOMBRE = vm.TipoProductoNomre,
                    TIPO_PRODUCTO_DESCRIPCION = vm.TipoProductoDescripcion,
                    ESTADO = vm.ESTADO,
                    ELIMINADO = false,
                ...
                _context.TIPO_PRODUCTO.Add(entidad);
                await _context.SaveChangesAsync(ct);
                await tx.CommitAsync(ct);

                // Flags para modal/toast de éxito
                TempData["SavedOk"] = true;
                TempData["SavedName"] = entidad.TIPO_PRODUCTO_NOMBRE;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(string id, TipoProductoViewModel vm)
        {
            if (string.IsNullOrWhiteSpace(id) || !string.Equals(id, vm.TipoProductoId, StringComparison.Ordinal))
                return NotFound();
            ...
            var entidad = await _context.TIPO_PRODUCTO.FirstOrDefaultAsync(p => p.TIPO_PRODUCTO_ID == id);
            if (entidad == null) return NotFound();

            // ====== Normalización de datos nuevos desde el VM ======
            var nuevoNombre = (vm.TipoProductoNomre ?? string.Empty).Trim();
            var nuevaDesc = vm.TipoProductoDescripcion?.Trim();
            //var nuevaAreaId = vm.AREA_ID;
            var nuevoEstado = vm.EstadoActivo ? "ACTIVO" : "INACTIVO";

            // ====== ¿Hay cambios? (nombre, desc, área, estado) ======
            var sinCambios =
                string.Equals(entidad.TIPO_PRODUCTO_NOMBRE ?? "", nuevoNombre, StringComparison.Ordinal) &&
                string.Equals(entidad.TIPO_PRODUCTO_DESCRIPCION ?? "", nuevaDesc ?? "", StringComparison.Ordinal) &&
            ...
            entidad.TIPO_PRODUCTO_NOMBRE = nuevoNombre;
            entidad.TIPO_PRODUCTO_DESCRIPCION = nuevaDesc;
            ...
            try
            {
                ...
                TempData["UpdatedName"] = entidad.TIPO_PRODUCTO_NOMBRE;
                return RedirectToAction(nameof(Edit), new { id });
            }
            catch (DbUpdateConcurrencyException)
            {
                if (!await _context.TIPO_PRODUCTO.AnyAsync(e => e.TIPO_PRODUCTO_ID == id)) return NotFound();
                throw;
            }
        }
        ...
        {
            var tIPO_PRODUCTO = await _context.TIPO_PRODUCTO.FindAsync(id);
            if (tIPO_PRODUCTO != null)
            {
                _context.TIPO_PRODUCTO.Remove(tIPO_PRODUCTO);
            }
        ...
        private bool TIPO_PRODUCTOExists(string id)
        {
            return _context.TIPO_PRODUCTO.Any(e => e.TIPO_PRODUCTO_ID == id);
        }
    }
}
