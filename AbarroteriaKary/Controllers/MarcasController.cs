...
namespace AbarroteriaKary.Controllers
{
    public class MarcasController : Controller
    {
        ...
        public MarcasController(KaryDbContext context, ICorrelativoService correlativos, IAuditoriaService auditoria)
        {
            _context = context;
        ...
        public async Task<IActionResult> Index(string? estado, string? q = null, string? fDesde = null, string? fHasta = null, int page = 1, int pageSize = 10)
        {
            ...
            if (!string.IsNullOrWhiteSpace(q))
            {
                var term = $"%{q.Trim()}%";
                qry = qry.Where(c =>
                    EF.Functions.Like(c.MARCA_ID, term) ||
                    EF.Functions.Like(c.MARCA_NOMBRE, term) ||
                    (c.MARCA_DESCRIPCION != null && EF.Functions.Like(c.MARCA_DESCRIPCION, term))
                );
            }
            ...
            var proyectado = qry
                .OrderBy(c => c.MARCA_NOMBRE)
                .Select(c => new MarcaViewModel
                {
                    MarcaId = c.MARCA_ID,
                    MarcaNombre = c.MARCA_NOMBRE,
                    MarcaDescripcion = c.MARCA_DESCRIPCION,
                    ESTADO = c.ESTADO,
        ...
        public async Task<IActionResult> Details(string id)
        {
            ...
            var entidad = await _context.MARCA
                .AsNoTracking()
                //.Include(p => p.AREA)
                .FirstOrDefaultAsync(r => r.MARCA_ID == id);

            if (entidad is null) return NotFound();

            // 3) Proyección a ViewModel (lo que usaremos en la vista)
            var vm = new MarcaViewModel
            {
                MarcaId = entidad.MARCA_ID,
                MarcaNombre = entidad.MARCA_NOMBRE,
                MarcaDescripcion = entidad.MARCA_DESCRIPCION,
        ...
        public async Task<IActionResult> Create(CancellationToken ct)
        {
            var vm = new MarcaViewModel
            {
                ...
                MarcaId = await _correlativos.PeekNextCategoriaIdAsync(ct),
                ESTADO = "ACTIVO",
                EstadoActivo = true,
        ...
        public async Task<IActionResult> Create(MarcaViewModel vm, CancellationToken ct)
        {
            // 1) Sincroniza checkbox -> cadena ESTADO
            vm.RefrescarEstadoDesdeBool();

            // 2) Normalización básica (evita espacios/valores nulos)
            vm.MarcaNombre = (vm.MarcaNombre ?? string.Empty).Trim();
            vm.MarcaDescripcion = vm.MarcaDescripcion?.Trim();

            // 3) Validación servidor
            if (!ModelState.IsValid)
            {
                // Reponer "preview" si se perdió
                if (string.IsNullOrWhiteSpace(vm.MarcaId))
                    vm.MarcaId = await _correlativos.PeekNextMarcaIdAsync(ct);

                return View(vm);
            }

            // 4) Regla de dominio: nombre único entre no eliminados
            var duplicado = await _context.MARCA.AnyAsync(c =>
                !c.ELIMINADO && c.MARCA_NOMBRE == vm.MarcaNombre, ct);

            if (duplicado)
            {
                ModelState.AddModelError(nameof(vm.MarcaNombre), "Ya existe una categoría con ese nombre.");
                if (string.IsNullOrWhiteSpace(vm.MarcaId))
                    vm.MarcaId = await _correlativos.PeekNextMarcaIdAsync(ct);
                return View(vm);
            }
            ...
            try
            {
                ...
                var nuevoId = await _correlativos.NextMarcaIdAsync(ct);

                var entidad = new MARCA
                {
                    MARCA_ID = nuevoId,
                    MARCA_NOMBRE = vm.MarcaNombre,
                    MARCA_DESCRIPCION = vm.MarcaDescripcion,
                    ESTADO = vm.ESTADO,
                    ELIMINADO = false,
                ...
                TempData["SavedName"] = entidad.MARCA_NOMBRE;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(string id, MarcaViewModel vm)
        {
            if (string.IsNullOrWhiteSpace(id) || !string.Equals(id, vm.MarcaId, StringComparison.Ordinal))
                return NotFound();
            ...
            var entidad = await _context.MARCA.FirstOrDefaultAsync(p => p.MARCA_ID == id);
            if (entidad == null) return NotFound();

            // ====== Normalización de datos nuevos desde el VM ======
            var nuevoNombre = (vm.MarcaNombre ?? string.Empty).Trim();
            var nuevaDesc = vm.MarcaDescripcion?.Trim();
            //var nuevaAreaId = vm.AREA_ID;
            var nuevoEstado = vm.EstadoActivo ? "ACTIVO" : "INACTIVO";

            // ====== ¿Hay cambios? (nombre, desc, área, estado) ======
            var sinCambios =
                string.Equals(entidad.MARCA_NOMBRE ?? "", nuevoNombre, StringComparison.Ordinal) &&
                string.Equals(entidad.MARCA_DESCRIPCION ?? "", nuevaDesc ?? "", StringComparison.Ordinal) &&
            ...
            entidad.MARCA_NOMBRE = nuevoNombre;
            entidad.MARCA_DESCRIPCION = nuevaDesc;
            ...
            try
            {
                ...
                TempData["UpdatedName"] = entidad.MARCA_NOMBRE;
                return RedirectToAction(nameof(Edit), new { id });
            }
            catch (DbUpdateConcurrencyException)
            {
                if (!await _context.MARCA.AnyAsync(e => e.MARCA_ID == id)) return NotFound();
                throw;
            }
        }
        ...
        {
            return _context.MARCA.Any(e => e.MARCA_ID == id);
        }
    }
}
