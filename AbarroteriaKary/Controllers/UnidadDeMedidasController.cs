...
namespace AbarroteriaKary.Controllers
{
    public class UnidadDeMedidasController : Controller
    {
        ...
        public UnidadDeMedidasController(KaryDbContext context, ICorrelativoService correlativos, IAuditoriaService auditoria)
        {
            _context = context;
        ...
        public async Task<IActionResult> Index(string? estado, string? q = null, string? fDesde = null, string? fHasta = null, int page = 1, int pageSize = 10)
        {
            ...
            var qry = _context.UNIDAD_MEDIDA
                .AsNoTracking()
                .Where(c => !c.ELIMINADO);
            ...
            if (!string.IsNullOrWhiteSpace(q))
            {
                var term = $"%{q.Trim()}%";
                qry = qry.Where(c =>
                    EF.Functions.Like(c.UNIDAD_MEDIDA_ID, term) ||
                    EF.Functions.Like(c.UNIDAD_MEDIDA_NOMBRE, term) ||
                    (c.UNIDAD_MEDIDA_DESCRIPCION != null && EF.Functions.Like(c.UNIDAD_MEDIDA_DESCRIPCION, term))
                );
            }
            ...
            var proyectado = qry
                .OrderBy(c => c.UNIDAD_MEDIDA_NOMBRE)
                .Select(c => new UnidaDeMedidaViewModel
                {
                    UnidadDeMedidaId = c.UNIDAD_MEDIDA_ID,
                    UnidadDeMedidaNombre = c.UNIDAD_MEDIDA_NOMBRE,
                    UnidadDeMedidaDescripcion = c.UNIDAD_MEDIDA_DESCRIPCION,
                    ESTADO = c.ESTADO,
        ...
        public async Task<IActionResult> Details(string id)
        {
            ...
            var entidad = await _context.UNIDAD_MEDIDA
                .AsNoTracking()
                //.Include(p => p.AREA)
                .FirstOrDefaultAsync(r => r.UNIDAD_MEDIDA_ID == id);

            if (entidad is null) return NotFound();

            // 3) Proyección a ViewModel (lo que usaremos en la vista)
            var vm = new UnidaDeMedidaViewModel
            {
                UnidadDeMedidaId = entidad.UNIDAD_MEDIDA_ID,
                UnidadDeMedidaNombre = entidad.UNIDAD_MEDIDA_NOMBRE,
                UnidadDeMedidaDescripcion = entidad.UNIDAD_MEDIDA_DESCRIPCION,
        ...
        public async Task<IActionResult> Create(CancellationToken ct)
        {
            var vm = new UnidaDeMedidaViewModel
            {
                ...
                UnidadDeMedidaId = await _correlativos.PeekNextUnidadDeMedidaIdAsync(ct),
                ESTADO = "ACTIVO",
                EstadoActivo = true,
        ...
        public async Task<IActionResult> Create(UnidaDeMedidaViewModel vm, CancellationToken ct)
        {
            // 1) Sincroniza checkbox -> cadena ESTADO
            vm.RefrescarEstadoDesdeBool();

            // 2) Normalización básica (evita espacios/valores nulos)
            vm.UnidadDeMedidaNombre = (vm.UnidadDeMedidaNombre ?? string.Empty).Trim();
            vm.UnidadDeMedidaDescripcion = vm.UnidadDeMedidaDescripcion?.Trim();

            // 3) Validación servidor
            if (!ModelState.IsValid)
            {
                // Reponer "preview" si se perdió
                if (string.IsNullOrWhiteSpace(vm.UnidadDeMedidaId))
                    vm.UnidadDeMedidaId = await _correlativos.PeekNextUnidadDeMedidaIdAsync(ct);

                return View(vm);
            }

            // 4) Regla de dominio: nombre único entre no eliminados
            var duplicado = await _context.UNIDAD_MEDIDA.AnyAsync(c =>
                !c.ELIMINADO && c.UNIDAD_MEDIDA_NOMBRE == vm.UnidadDeMedidaNombre, ct);

            if (duplicado)
            {
                ModelState.AddModelError(nameof(vm.UnidadDeMedidaNombre), "Ya existe una Medida con ese nombre.");
                if (string.IsNullOrWhiteSpace(vm.UnidadDeMedidaId))
                    vm.UnidadDeMedidaId = await _correlativos.PeekNextUnidadDeMedidaIdAsync(ct);
                return View(vm);
            }
            ...
            try
            {
                ...
                var nuevoId = await _correlativos.NextUnidadDeMedidaIdAsync(ct);

                var entidad = new UNIDAD_MEDIDA
                {
                    UNIDAD_MEDIDA_ID = nuevoId,
                    UNIDAD_MEDIDA_NOMBRE = vm.UnidadDeMedidaNombre,
                    UNIDAD_MEDIDA_DESCRIPCION = vm.UnidadDeMedidaDescripcion,
                    ESTADO = vm.ESTADO,
                    ELIMINADO = false,
                ...
                _context.UNIDAD_MEDIDA.Add(entidad);
                await _context.SaveChangesAsync(ct);
                await tx.CommitAsync(ct);

                // Flags para modal/toast de éxito
                TempData["SavedOk"] = true;
                TempData["SavedName"] = entidad.UNIDAD_MEDIDA_NOMBRE;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(string id, UnidaDeMedidaViewModel vm)
        {
            if (string.IsNullOrWhiteSpace(id) || !string.Equals(id, vm.UnidadDeMedidaId, StringComparison.Ordinal))
                return NotFound();
            ...
            var entidad = await _context.UNIDAD_MEDIDA.FirstOrDefaultAsync(p => p.UNIDAD_MEDIDA_ID == id);
            if (entidad == null) return NotFound();

            // ====== Normalización de datos nuevos desde el VM ======
            var nuevoNombre = (vm.UnidadDeMedidaNombre ?? string.Empty).Trim();
            var nuevaDesc = vm.UnidadDeMedidaDescripcion?.Trim();
            //var nuevaAreaId = vm.AREA_ID;
            var nuevoEstado = vm.EstadoActivo ? "ACTIVO" : "INACTIVO";

            // ====== ¿Hay cambios? (nombre, desc, área, estado) ======
            var sinCambios =
                string.Equals(entidad.UNIDAD_MEDIDA_NOMBRE ?? "", nuevoNombre, StringComparison.Ordinal) &&
                string.Equals(entidad.UNIDAD_MEDIDA_DESCRIPCION ?? "", nuevaDesc ?? "", StringComparison.Ordinal) &&
            ...
            entidad.UNIDAD_MEDIDA_NOMBRE = nuevoNombre;
            entidad.UNIDAD_MEDIDA_DESCRIPCION = nuevaDesc;
            ...
            try
            {
                ...
                TempData["UpdatedName"] = entidad.UNIDAD_MEDIDA_NOMBRE;
                return RedirectToAction(nameof(Edit), new { id });
            }
            catch (DbUpdateConcurrencyException)
            {
                if (!await _context.UNIDAD_MEDIDA.AnyAsync(e => e.UNIDAD_MEDIDA_ID == id)) return NotFound();
                throw;
            }
        }
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            var uNIDAD_MEDIDA = await _context.UNIDAD_MEDIDA
                .FirstOrDefaultAsync(m => m.UNIDAD_MEDIDA_ID == id);
        ...
        {
            var uNIDAD_MEDIDA = await _context.UNIDAD_MEDIDA.FindAsync(id);
            if (uNIDAD_MEDIDA != null)
            {
                _context.UNIDAD_MEDIDA.Remove(uNIDAD_MEDIDA);
            }
        ...
        {
            return _context.UNIDAD_MEDIDA.Any(e => e.UNIDAD_MEDIDA_ID == id);
        }
    }
}
